...
rain_paths = [
    "/data/shared_data/NNA/NNA.4km.hERA5.1989.003/rain_d/*.nc",
    "/data/shared_data/NNA/NNA.4km.fPGWh.2033.004/rain_d/*.nc",
    "/data/shared_data/NNA/NNA.4km.fPGWm.2033.005/rain_d/*.nc",
]
...
def precip_by_gridcells(snow_ds, rain_ds, swe_ds, year_range=(1989, 1990), scenario_flag="P"):
    def one_year_range(year_start, year_end, scenario_type):
        ...
        rain_slice = rain_ds.sel(time=slice(f"{year_start}-01-01", f"{year_end}-12-31"))
        precip_slice = snow_slice + rain_slice
        doy_broadcast = xr.broadcast(doy_axis, precip_slice.isel(time=0))[0]

        mask = (doy_broadcast >= min_doy) & (doy_broadcast <= max_doy)
        masked_precip = precip_slice.where(mask) * 86400
        total_precip = masked_precip.sum("time", skipna=True)

        precip_ratio = (max_swe - min_swe) / total_precip
        print(f"math complete for {year_start}-{year_end}_{scenario_type}")

        output_dir = "outputs_nuevo_precip" # change if needed

        os.makedirs(output_dir, exist_ok=True)
        out_base = f"{output_dir}/precipratio_{year_start}_{year_end}_{scenario_type}"
        # this following line is a dumb trick to give us times to sort over.
        precip_ratio = precip_ratio.expand_dims(precip_year=[f"{year_start}-{year_end}"])
        precip_ratio.name = "precip_ratio"
        precip_ratio.to_netcdf(f"{out_base}.nc")
        print(f"netcdf export complete for {year_start}-{year_end}_{scenario_type}")

        plt.figure(dpi=300)
        precip_ratio.plot(vmax=1)
...
def main():
    ...
    # seriously, need to get this raised
    # TODO: bump that ulimit
    
    final_stack_P = precip_by_gridcells(snow_P, rain_P, total_better_h2osno_P, (1990, 2020), "P")
    final_stack_H = precip_by_gridcells(snow_H, rain_H, total_better_h2osno_H, (2034, 2064), "H")
    final_stack_M = precip_by_gridcells(snow_M, rain_M, total_better_h2osno_M, (2034, 2064), "M")
