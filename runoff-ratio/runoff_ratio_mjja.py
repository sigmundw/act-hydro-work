...
y_origin = summerizer(total_temp_P, f"summerized_temp_P_{magic}", scale_by_time=False).mean().values
...
precip_ratio_values_P = [(summerizer(total_qrunoff_P, f"summerized_qrunoff_P_{magic}").sel(season_year=x) / summerizer(total_precip_P, f"summerized_precip_P_{magic}").sel(season_year=x)).item()
                         for x in years_P]
# y axis 
temp_values_P = [(summerizer(total_temp_P, f"summerized_temp_P_{magic}", scale_by_time=False).sel(season_year=x)).item() - y_origin
                 for x in years_P]
# x axis
...
runoff_values_debug_P = [(summerizer(total_qrunoff_P, f"summerized_qrunoff_P_{magic}").sel(season_year=x)).item()
                         for x in years_P]
# y axis 
temp_values_debug_P = [(summerizer(total_temp_P, f"summerized_temp_P_{magic}", scale_by_time=False).sel(season_year=x)).item()
                       for x in years_P]
# x axis
...
# y axis 
temp_values_H = [(summerizer(total_temp_H, f"summerized_temp_H_{magic}", scale_by_time=False).sel(season_year=x).values).item() - y_origin
                 for x in years_H]
# x axis
...
runoff_values_debug_H = [(summerizer(total_qrunoff_H, f"summerized_qrunoff_H_{magic}").sel(season_year=x).values).item()
                         for x in years_H]
# y axis 
temp_values_debug_H = [(summerizer(total_temp_H, f"summerized_temp_H_{magic}", scale_by_time=False).sel(season_year=x).values).item()
                       for x in years_H]
# x axis
...
precip_ratio_values_M = [(summerizer(total_qrunoff_M, f"summerized_qrunoff_M_{magic}").sel(season_year=x).values / summerizer(total_precip_M, f"summerized_precip_M_{magic}").sel(season_year=x).values).item()
           for x in years_M]
# y axis 
temp_values_M = [(summerizer(total_temp_M, f"summerized_temp_M_{magic}", scale_by_time=False).sel(season_year=x).values).item() - y_origin
           for x in years_M]
# x axis
...
runoff_values_debug_M = [(summerizer(total_qrunoff_M, f"summerized_qrunoff_M_{magic}").sel(season_year=x).values).item()
                         for x in years_M]
# y axis 
temp_values_debug_M = [(summerizer(total_temp_M, f"summerized_temp_M_{magic}", scale_by_time=False).sel(season_year=x).values).item()
                       for x in years_M]
# x axis
...
## Debugging symbols
log_file = open(f"runoff_ratio/plot-{region_dic[magic]}-MJJA.log.txt", 'w')
...
historical_file_name = f"runoff_ratio/{region_dic[magic]}_mjja_hist_data.nc"
future_file_name = f"runoff_ratio/{region_dic[magic]}_mjja_future_data.nc"
...
plt.suptitle(f"Runoff / Precipitation Ratio for Different Precipitation and Temperature Values and Situations (MJJA) ({region_dic[magic]})")
...
plt.savefig(f"runoff_ratio/plot-{region_dic[magic]}-MJJA.png")
